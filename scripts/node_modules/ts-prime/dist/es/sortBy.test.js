...
describe('data first', function () {
    ...
    test('sort with multi keys', function () {
        expect(sortBy(multiPropSortItems, function (x) { return [{ value: x.favorite, order: 'desc' }, x.category]; })).toEqual(multiPropSortItemsSorted);
    });
    test('sort with multi keys', function () {
        expect(sortBy(withLocaleComparison, function (x) { return [{ value: x.category, compare: function (a, b) { return a.toString().localeCompare(b.toString()); } }, x.category]; })).toEqual(sortedWithLocaleComparison);
    });
});
